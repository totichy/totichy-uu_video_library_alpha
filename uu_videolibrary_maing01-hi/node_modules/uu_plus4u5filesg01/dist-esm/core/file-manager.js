...
import { adjustFile, getBinaryData, getMaxFileSize, isImage } from "../helpers/file.js";
...
const FileManager = UU5.Common.VisualComponent.create({
  ...
  _buildBinaryJson(dtoOut) {
    let type = this.props.onSetFileType ? this.props.onSetFileType(dtoOut) : getBinaryData(dtoOut);
    let binaryObj = {};
  ...
  _validateCodeUniqueness(code) {
    return !this.state.binaries.some(binary => binary.code === code);
  },
  ...
  _onSetFiles(fileList, opt) {
    ...
    let hasImage = fileList.some(file => isImage(file));

    if (
      customCodesEnabled ||
      this.props.availableTags ||
      this.props.allowCustomTags ||
      (customQualityEnabled && hasImage)
    ) {
      this._openDnDForm(
        fileList,
        (outputFileList, codeList, tagList) => {
          this._uploadFiles(outputFileList, codeList, tagList, false);
        },
