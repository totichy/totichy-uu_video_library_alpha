...
import { adjustFile, isImage } from "../helpers/file.js";
...
const FileUploader = UU5.Common.VisualComponent.create({
  ...
  getInitialState() {
    ...
    this._lsiLabels = {
      ...
      addBinaryFormQuality: Lsi.LSI_LABEL_ADD_BINARY_QUALITY,
      addBinaryFormPreserveDimensions: Lsi.LSI_LABEL_ADD_BINARY_PRESERVE_DIMENSIONS
    };
  ...
  async setFiles(files, opt) {
    ...
    if (files.length > 0) {
      ...
      if (this.props.onSetFiles && typeof this.props.onSetFiles === "function") {
        outputFiles = await this._adjustFiles(files);
        this._files = outputFiles;
        this.props.onSetFiles(outputFiles, opt);
      } else {
        let hasImage = files.some(file => isImage(file));
        let customQualityEnabled = this.props.outputQuality == null;
        if (this.props.customCodesEnabled || (customQualityEnabled && hasImage)) {
          this._files = outputFiles;
          this._openDnDForm(outputFiles);
        } else {
          outputFiles = await this._adjustFiles(files);
          this._files = outputFiles;
          this._uploadFiles(outputFiles);
        }
      }
    }
  ...
  async _adjustFiles(files) {
    let adjustedFiles = [];
    for (let i = 0; i < files.length; i++) {
      let adjustedFile = await adjustFile(files[i], this.props);
  ...
  _openDnDForm(fileList) {
    this._formController.openDnDForm(
      fileList,
      (outputFileList, codeList) => {
        this._files = outputFileList;
        this._codes = codeList;
        this._uploadFiles(outputFileList);
      },
      {
