...
class UuDataStoreDao {
  ...
  async lockOne(filter, lock) {
    ...
    if (resultObject.length > 0) {
      ...
      return await db
        .collection(this.collectionName)
        .findOneAndUpdate(lockFilter, update, { returnOriginal: false })
        .then((result) => {
  ...
  async unlockOne(filter, lock) {
    ...
    if (!(lock instanceof Lock)) {
      ...
      try {
        ...
        return await db
          .collection(this.collectionName)
          .findOneAndUpdate(filter, unlockEdit, { returnOriginal: false })
          .then((result) => {
            if (result.value === null) {
  ...
  async _recalculateCurrentNoiAfter(change, awid) {
    ...
    if (this.noiCounts[awid] === undefined) this._countNoi(awid);
