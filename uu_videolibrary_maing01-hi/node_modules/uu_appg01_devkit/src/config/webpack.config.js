...
function getWebpackConfig(options, env) {
  ...
  if (opts.copyFiles) {
    ...
    if (templateType === "uu5-app" || (opts.includeDemoPages && templateType === "uu5-lib")) {
      ...
      if (!fs.existsSync(path.join(outputAbsPath, "callbacks"))) {
        let uuAppOidcDir = getModulePathFromProject("uu_appg01_oidc");
        if (uuAppOidcDir) {
          cwpSettings.push({
          ...
          let uuOidcDir = getModulePathFromProject("uu_oidcg01");
          if (uuOidcDir) {
            cwpSettings.push({ context: path.join(uuOidcDir, "dist"), from: "callbacks/**" });
          }
        }
      }
    }
  ...
  let webpackOutputFileName =
    opts.outputFile && opts.outputFile.match(/\.(css|less)$/)
      ? path.join(path.relative(outputAbsPath, paths.buildWebpackTempDir), "__ignored", require("uuid/v4")())
  ...
  if (reactHot && opts.copyFiles) {
    ...
    // react-dom might not be in app's node_modules if using pnpm => look for it in uu5g04/uu5g05's too
    let reactDomPath = getModulePathFromProject("react-dom");
    if (!reactDomPath) {
...
function getModulePathFromDirectory(moduleName, dir) {
  let result;
  try {
    let pkgPath = require.resolve(moduleName + "/package.json", { paths: [dir] });
    result = path.dirname(pkgPath);
  } catch (e) {
    result = null;
  }
  return result;
