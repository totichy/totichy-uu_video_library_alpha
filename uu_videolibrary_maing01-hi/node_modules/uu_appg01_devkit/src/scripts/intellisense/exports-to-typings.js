...
class ExportsToTypings {
  ...
  _serializeField(name, value = {}) {
    let members = [];
    let serializedName = name === "default" ? "_default" : name;
    let prevented = this._preventLoop(value, () => {
    ...
    if (prevented) {
      if (value.type === "namespace") members.push(this._serializeNamespace(name, {}));
      else members.push(dom.create.const(serializedName, dom.type.any));
    }
    return members;
  }

  _preventLoop(value, runnableFn) {
    if (!this._inFlightValues) this._inFlightValues = new WeakSet();
    let preventable = value && (typeof value === "object" || typeof value === "function");
    if (preventable && this._inFlightValues.has(value)) return true;
    if (preventable) this._inFlightValues.add(value);
    try {
      runnableFn();
    } finally {
      if (preventable) this._inFlightValues.delete(value);
    }
    return false;
  }
