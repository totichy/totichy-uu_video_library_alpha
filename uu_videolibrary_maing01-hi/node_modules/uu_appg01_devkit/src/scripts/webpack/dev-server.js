...
module.exports = class DevServer {
  ...
  start() {
    ...
    if (Object.keys(mappings || {}).length > 0) {
      ...
      const RESERVED_URL_PATTERNS = [/\/oidc\/(callback|getClientId)(?:\?|$)/];
      app.use((req, res, next) => {
        if (
          req.method == "GET" &&
          (!appAssetsRelativeUrlPath || !req.url.startsWith("/" + appAssetsRelativeUrlPath)) &&
          !RESERVED_URL_PATTERNS.some((it) => req.url.match(it))
        ) {
    ...
    // make OIDC callbacks available from anywhere for demo pages / uuApps running only with frontend web server
