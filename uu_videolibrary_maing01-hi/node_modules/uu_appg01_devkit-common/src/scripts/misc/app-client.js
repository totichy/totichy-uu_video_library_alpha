...
class AppClient {
  constructor(token, opts = {}) {
    this.token = token;
    this.os8BaseUri = ConfigHelper.getConfigParam('uu_os8_server_uri') || opts.uuOs8ServerUri || DEFAULT_OS8_BASE_URI;
    this.c3BaseUri = ConfigHelper.getConfigParam('uu_c3_server_uri') || opts.uuC3ServerUri || DEFAULT_C3_BASE_URI;
    this.logstoreBaseUri = ConfigHelper.getConfigParam('uu_logstore_server_uri') || opts.uuLogStoreServerUri || DEFAULT_LOGSTORE_BASE_URI;
  }

  async exchange(url, method, params, headers = DEFAULT_HEADERS, tryNumber = 0, gotOpts = {}) {
    ...
    try {
      ...
      if (e.statusCode == 401 || (e.response && e.response.body.toString().indexOf("UU.OS.APPLICATION/E003-NOT_AUTHENTICATED") > 0)) {
        ...
        return await this.exchange(url, method, params, headers, ++tryNumber, gotOpts);
