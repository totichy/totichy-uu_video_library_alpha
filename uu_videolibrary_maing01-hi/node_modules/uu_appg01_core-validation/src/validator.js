...
class Validator {
  ...
  _validateInternal(value, ctx, result, chainObject) {
    ...
    validator.matchers.forEach(matcher => {
      let partialResult = matcher.match(value, ctx, this.validatorMap);
      let valid = result.valid && partialResult.valid;
      Object.assign(result, partialResult, { valid });
    });
  ...
  validate(type, value) {
    ...
    if (!finalResult.invalidTypes) {
      finalResult.invalidTypes = {};
    }
    if (!finalResult.invalidValues) {
      finalResult.invalidValues = {};
    }
    if (!finalResult.missingKeys) {
      finalResult.missingKeys = {};
    }
    if (!finalResult.invalidKeyMap) {
      finalResult.invalidKeyMap = {};
    }
    if (!finalResult.unsuportedKeys) {
      finalResult.unsuportedKeys = [];
    }
    if (!finalResult.childResults) {
      finalResult.childResults = {};
    }
    return finalResult;
  }
...
let isNode = new Function("try {return !!(typeof process !== undefined && process && process.versions && process.versions.node);}catch(e){return false;}");
