...
const {BaseError, LruCache, Base64, Config} = require("uu_appg01_core-utils");
...
const LocalDomainHandler = require("./local-domain-handler.js");
...
class OAuthClient {
  ...
  constructor(serviceUri, opts = {}) {
    ...
    this._tokenCache = new LruCache({maxSize: tokenCacheSize});
  ...
  async introspectToken(token, tokenType = null, clientCredentials = null) {
    ...
    let tokenTypeHint = tokenType || this._getTokenTypeHint(token);
    if (tokenTypeHint) {
  ...
  _getOidcScope(scope, allowDevelScope = false) {
    let oidcScope = null;
    if (Array.isArray(scope)) {
      ...
      oidcScope = `${this._defaultScope} ${LocalDomainHandler.getLocalDomainScope()}`;
    } else {
