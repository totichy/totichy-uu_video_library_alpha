...
const AbstractAuthorizationFlow = require("./internal/abstract-authorization-flow.js");
const ImplicitFlow = require("./internal/implicit-flow.js");
const CodeFlow = require("./internal/code-flow.js");
const CodeFlowV2 = require("./internal/code-flow-v2.js");
...
const LocalDomainHandler = require("../internal/local-domain-handler.js");
  ...
  async restoreSession(options = {}) {
    ...
    if (!this._restoreSessionPromise) {
      ...
      if (session && session.matches(acrValues || this._defaultAcrValues, maxAge || this._defaultMaxAge)) {
        ...
        restoreSessionArgs = {acrValues, maxAge};
      }
  ...
  async _authenticateByCallToken(callToken) {
    let mockRunningAuthPromise = async function() {
      ...
      if (callToken.match(JWT_TOKEN_REGEX)) {
        ...
        let appClient = new AppClient({baseUri: Dom.canonicalAppBaseUri || Dom.appBaseUri});
  ...
  async _doAuthenticateRefresh() {
    try {
      ...
      if (this._authnFlow.refreshSession) {
        result = await this._authnFlow.refreshSession({ acrValues, maxAge }, scope);
      } else {
        result = await this._authnFlow.authenticate({ acrValues, maxAge, prompt: "none", usePopup: false }, scope);
      }
