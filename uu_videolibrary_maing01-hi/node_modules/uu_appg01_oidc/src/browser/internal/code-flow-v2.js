...
class CodeFlowV2 extends CodeFlow {
  ...
  async checkSession(query) {
    ...
    if (this._doOnlyLocalSessionCheck) {
      ...
      if (tokenParts.length === 3) {
        ...
        return Number(tokenParts[0]) * 1000 > Date.now() ? "unchanged" : "changed";
      } else {
  ...
  async refreshSession(options, scope = null) {
    ...
    let claims  = (await this._client.post(this._appBaseUri + "oidc/extendSession")).data;
  ...
  _throwInteractionRequired() {
    let error = new AuthenticationError("Authentication Request cannot be completed without displaying a user interface for End-User interaction.");
