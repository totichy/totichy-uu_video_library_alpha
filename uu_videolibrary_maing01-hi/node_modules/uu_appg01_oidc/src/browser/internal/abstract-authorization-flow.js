...
const { Config, Base64, BaseError } = require("uu_appg01_core-utils");
...
class AbstractAuthorizationFlow {
  ...
  authenticate(options, scope = null) {
    ...
    let resultPromise = new Promise(async (resolve, reject) => {
      try {
        let { acrValues, maxAge, prompt, language, usePopup, authenticationContext, loginHint, noIframe} = options || {};
        ...
        if (prompt === "none") {
          ...
          authUri = await this._oauthClient.getAuthorizationUri(this.getResponseType(), this._defaultRedirectUri, clientCredentials, acrValues, scope);
        } else {
          try {
          ...
          try {
            authUri = this._oauthClient.getAuthorizationUriSync(this.getResponseType(), this._defaultRedirectUri, clientCredentials, acrValues, scope);
          } catch (e) {
            if (e instanceof SyncCallNotPossible) {
  ...
  async getCallToken(options, scope = null) {
    let {idToken, accessToken, claims} = await this.authenticate(options, scope);
    let expireAt = claims.exp * 1000;
    return {idToken, accessToken, expireAt};
  }
