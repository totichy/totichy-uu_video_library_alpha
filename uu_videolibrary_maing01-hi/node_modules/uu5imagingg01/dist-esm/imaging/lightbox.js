...
export const Lightbox = UU5.Common.VisualComponent.create({
  ...
  componentWillMount() {
    ...
    if (!this._isTargetLightbox() && data) {
      ...
      data.forEach(item => {
        targetLightbox._addImage(item.src, item.id, item.authenticate, item.sourceComponent, this, null, item.alt);
      });
    }
  },
    ...
    if (this.props.data !== nextProps.data) {
      ...
      targets.forEach(target => {
        ...
        newData.forEach(item => {
          target._addImage(item.src, item.id, item.authenticate, item.sourceComponent, this, true, item.alt);
        });
      });
    }
  ...
  //@@viewOn:interface
  addImage(src, id, authenticate, sourceComponent, alt) {
    if (id === undefined) {
      id = src;
    }
  ...
  //@@viewOn:private
  _addImage(src, id, authenticate, sourceComponent, sourceLightbox = this, ignoreTarget, alt) {
    if (id === undefined) {
      id = src;
    }

    if (ignoreTarget || this._isTargetLightbox()) {
      this._options.data.push({ id, src, authenticate, sourceComponent, sourceLightbox, alt });
    } else {
